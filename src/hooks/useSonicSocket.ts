...
export function useSonicSocket() {
    ...
    */
    ...
    const sendCommand = useCallback(async (text: string) => {
        ...
        const isDirectCommand = /^(drums|bass|melody|fx|bpm):/.test(trimmed);
        const isRawCode = /^(s\(|note\(|stack\(|silence|sound\(|sample\(|n\(|m\(|\(\(\) =>)/.test(trimmed);

        if (isDirectCommand || isRawCode) {
            console.log('[SonicSocket] Sending direct command:', trimmed);
            if (!socketRef.current?.connected) {
